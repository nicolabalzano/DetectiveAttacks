...
sw_NN = set(
    list(stop_words) + ['part', 'kind', 'different', 'number', 'all', 'many', 'kinds', 'parts', 'whole', 'certain',
                        'various', 'other', 'such', 'both', 'multiple', 'some', 'several'])
...
class NLP:
    ...
    def extract_VO_from_docs_lambda(self, doc_srl, arg_constrain={}, join_args=True,
                                    return_args=['V', 'ARG1', 'ARG2', 'ARG3', 'ARGM-LOC', 'ARGM-EXT']):
        out = {}
        exclude = []
        for doc in doc_srl:
            out[doc] = []
                    ...
                    if 'V' in doc_srl[doc][sent][v]:
                        ...
                        for arg in arg_constrain:
                            if arg == 'forbid':
                                ...
                                break
                        if cont:
                            args = [doc_srl[doc][sent][v][arg]['text'] for arg in doc_srl[doc][sent][v] if
                                    arg in return_args]
                ...
                for v in doc_srl[doc][sent]:
                    ...
                    for arg in arg_constrain:
                        if arg == 'forbid':
                            ...
                            break
                    if cont:
                        out[doc][sent][v] = {i: doc_srl[doc][sent][v][i] for i in doc_srl[doc][sent][v] if
                                             return_args == None or i in return_args}
        return out

    def extract_VO_from_sents_lambda(self, sents_srl, arg_constrain={}, join_args=True,
                                     return_args=['V', 'ARG1', 'ARG2', 'ARG3', 'ARGM-LOC', 'ARGM-EXT']):
        out = {}
        exclude = []
    ...
    def extract_VO_from_sents(self, sents_srl, arg_constrain={}, join_args=True,
                              return_args=['V', 'ARG1', 'ARG2', 'ARG3', 'ARGM-LOC', 'ARGM-EXT']):
        out = {}
        for sent in sents_srl:
            out[sent] = []
    ...
    def extract_VO_from_srl(srl, sent, arg_constrain={}, join_args=True,
                            return_args=['V', 'ARG1', 'ARG2', 'ARG3', 'ARGM-LOC', 'ARGM-EXT']):
        exclude = []
